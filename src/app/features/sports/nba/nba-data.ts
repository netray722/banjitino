...
export function normalizeNbaStandings(payload: unknown): NbaStanding[] {
  ...
  return (root.children ?? []).flatMap((group) => {
    const conference = nbaConference(group.name ?? group.abbreviation ?? '');
    if (!conference) return [];
    return (group.standings?.entries ?? []).flatMap((entry) => {
      const teamId = Number.parseInt(entry.team?.id ?? '', 10) || 0;
      const teamCode = nbaCode(entry.team?.abbreviation ?? '');
      if (!teamId && !teamCode) return [];
      return [{
        teamId,
        teamCode,
        conference,
        seed: standingValue(entry, 'playoffSeed'),
        wins: standingValue(entry, 'wins'),
        losses: standingValue(entry, 'losses'),
        winPercentage: standingValue(entry, 'winPercent'),
        gamesBehind: standingValue(entry, 'gamesBehind')
      }];
    });
  });
}

export function buildNbaStandingsLookup(standings: NbaStanding[]): NbaStandingsLookup {
  const lookup: NbaStandingsLookup = {};
  for (const standing of standings) {
    if (standing.teamId) lookup[`id:${standing.teamId}`] = standing;
    if (standing.teamCode) lookup[`code:${standing.teamCode}`] = standing;
  }
  return lookup;
}

export function findNbaStanding(lookup: NbaStandingsLookup, team: TeamSummary): NbaStanding | null {
...
export function normalizeScoreboard(payload: unknown): Scoreboard {
  ...
  const root = payload as NbaScoreboardPayload;
...
function normalizeEspnScoreboard(payload: unknown): Scoreboard | null {
  const root = payload as EspnScoreboardPayload;
...
function normalizeStatsScoreboard(payload: unknown): Scoreboard | null {
  const root = payload as StatsScoreboardPayload;
...
export function normalizeBoxScore(payload: unknown): BoxScore {
  ...
  const root = payload as NbaBoxScorePayload;
...
function normalizeEspnBoxScore(payload: unknown): BoxScore | null {
  const root = payload as EspnBoxScorePayload;
